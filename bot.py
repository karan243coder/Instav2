...
def run_yt_dlp_download(url: str, cookiefile: Optional[str] = None) -> Tuple[Optional[dict], List[str], Optional[str]]:
    ...
    Handles: single video, single photo, carousel posts (multiple images)
    """
    ...
    opts = {
        ...
        "quiet": True,
        "no_warnings": True,
        "format": "best/bestvideo+bestaudio",  # best handles both photos and videos
        "max_filesize": 50 * 1024 * 1024,
        "merge_output_format": "mp4",
        "playlist_items": "1-10",  # for carousel posts
    }
    ...
    try:
        with yt_dlp.YoutubeDL(opts) as ydl:
            ...
            # Discover ALL media files in output directory
            media_files = []
            if os.path.exists(out_dir):
                for f in sorted(os.listdir(out_dir)):
                    ...
                    if not os.path.isfile(fpath):
                        continue
                    # Skip temp/meta files
                    if f.endswith((".part", ".json", ".description", ".ytdl", ".temp")):
                        continue
                    # Only include media files > 1KB
...
# ==================== MESSAGE HANDLER (PHOTO + VIDEO + STORY FIXED) ====================
async def handle_text(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if "tiktok.com" in text.lower() or "tiktok" in text.lower():
        ...
        info, files, err = await asyncio.get_event_loop().run_in_executor(executor, run_yt_dlp_download, text, None)
        if files:
            ...
            for f in files:
                if await safe_send_media(update, f, caption if sent == 0 else ""):
                    sent += 1
            if sent > 0 and files:
                await log_download_success(update, context, text, files[0], info, is_tiktok=True)
            else:
    ...
    if "instagram.com" in text.lower() or "instagr.am" in text.lower():
        ...
        info, files, err = await asyncio.get_event_loop().run_in_executor(executor, run_yt_dlp_download, text, COOKIE_FILE)
        ...
        if files:
            ...
            if sent > 0 and files:
                await log_download_success(update, context, text, files[0], info, is_tiktok=False)
            else:
                await update.message.reply_text("❌ File send nahi ho payi.")
